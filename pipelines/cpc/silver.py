...
Uso:
  ...
  python -m pipelines.cpc.silver --year 2021-2023
"""
...
def _find_xlsx(year: int) -> Path:
    """Localiza o arquivo XLSX do CPC para o ano dado."""
    ...
    candidates = list(base.glob("*.xlsx")) + list(base.glob("*.XLSX")) + list(base.glob("*.xls"))
    if not candidates:
...
def _read_xlsx(path: Path) -> pl.DataFrame:
    """
    Lê o XLSX do CPC.
    - Detecta a aba principal (ignora 'Atualizações')
    - Remove espaços em branco dos nomes de colunas
    - Converte células para string para evitar tipos mistos
    - Descarta linhas de rodapé onde 'Código da IES' não é numérico
    """
    import openpyxl
    ...
    # Header na primeira linha — normaliza espaços
    header = [str(h).strip() if h is not None else f"col_{i}" for i, h in enumerate(rows[0])]
    data = rows[1:]

    # Tudo como string — cast correto feito depois
    str_data = {
        col: [
...
def _rename_columns(df: pl.DataFrame) -> pl.DataFrame:
    ...
    existing = {k: v for k, v in CPC_COLUMN_MAP.items() if k in df.columns}
...
def process_year(year: int, verbose: bool = False, force: bool = False) -> None:
    ...
    assert_required_columns(df, CPC_REQUIRED_COLUMNS, f"cpc/{year}")
    assert_no_nulls(df, ["co_ies", "co_curso", "ano"], f"cpc/{year}")
...
@click.command()
@click.option("--year", required=True, help="Ano ou intervalo: 2023 ou 2021-2023")
